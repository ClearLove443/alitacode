...
export async function loadConfig(
  ...
  const dir = configDir ?? path.join(os.homedir(), ".alita");
...
export function createJournalStore(configDir?: string): JournalStore {
  const journalDir = path.join(
    configDir ?? path.join(os.homedir(), ".alita"),
    "journal",
  );
  ...
  return {
    ...
    async search(query) {
      ...
      if (query.text) {
        log.info("Starting semantic search for:", { text: query.text });
        try {
          queryEmbedding = await generateEmbedding(query.text);
          log.info("Query embedding generated successfully");
        } catch (error) {
          log.warn("Failed to generate query embedding, falling back to text search", { error });
      ...
      for (const file of files) {
        ...
        if (query.text) {
          if (queryEmbedding) {
            // Semantic search
            log.info("Loading embedding for entry:", { file });
            const entryEmbedding = await loadEmbedding(filePath);
            if (entryEmbedding) {
              score = cosineSimilarity(queryEmbedding, entryEmbedding);
              log.info("Semantic similarity calculated", { score });
            } else {
              // No embedding stored; fall back to text match
              log.info("No embedding found for entry, falling back to text match");
              const haystack =
            ...
            log.info("Using text search fallback");
